...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def register_actor(actor):
        ...
        if actor in CarlaDataProvider._actor_velocity_map:
            raise KeyError(
                "Vehicle '{}' already registered. Cannot register twice!".format(
                    actor.id
                )
            )
        else:
    ...
    def update_light_states(
        ego_light, annotations, states, freeze=False, timeout=1000000000
    ):
        """
    ...
    def create_blueprint(
        model, rolename="scenario", color=None, actor_category="car", safe=False
    ):
        """
        ...
        try:
            ...
            if new_model != "":
                bp_filter = new_model
            print(
                "WARNING: Actor model {} not available. Using instead {}".format(
                    model, new_model
                )
            )
    ...
    def request_new_actor(
        ...
        if actor is None:
            print(
                "WARNING: Cannot spawn actor {} at position {}".format(
