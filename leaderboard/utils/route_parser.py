...
class RouteParser(object):
    ...
    def parse_routes_file(route_filename, scenario_file, single_route_id=""):
        ...
        print("list_route_descriptions: ", list_route_descriptions)
            ...
            for weather_attrib in route.iter("weather"):
                ...
                if "precipitation" in weather_attrib.attrib:
                    weather.precipitation = float(
                        weather_attrib.attrib["precipitation"]
                    )
                if "precipitation_deposits" in weather_attrib.attrib:
                    weather.precipitation_deposits = float(
                        weather_attrib.attrib["precipitation_deposits"]
                    )
                if "wind_intensity" in weather_attrib.attrib:
                    weather.wind_intensity = float(
                        weather_attrib.attrib["wind_intensity"]
                    )
                if "sun_azimuth_angle" in weather_attrib.attrib:
                    weather.sun_azimuth_angle = float(
                        weather_attrib.attrib["sun_azimuth_angle"]
                    )
                if "sun_altitude_angle" in weather_attrib.attrib:
                    weather.sun_altitude_angle = float(
                        weather_attrib.attrib["sun_altitude_angle"]
                    )
    ...
    def check_trigger_position(new_trigger, existing_triggers):
        ...
        for trigger_id in existing_triggers.keys():
            ...
            if distance < TRIGGER_THRESHOLD and (
                dyaw < TRIGGER_ANGLE_THRESHOLD or dyaw > (360 - TRIGGER_ANGLE_THRESHOLD)
            ):
                return trigger_id
    ...
    def match_world_location_to_route(world_location, route_description):
        ...
        def match_waypoints(waypoint1, wtransform):
            ...
            return (
                dz < TRIGGER_THRESHOLD
                and dpos < TRIGGER_THRESHOLD
                and (
                    dyaw < TRIGGER_ANGLE_THRESHOLD
                    or dyaw > (360 - TRIGGER_ANGLE_THRESHOLD)
                )
            )
                ...
                for event in scenario["available_event_configurations"]:
                    ...
                    if match_position is not None:
                        ...
                        if "other_actors" in event:
                            ...
                            other_vehicles = None
                        scenario_subtype = RouteParser.get_scenario_type(
                            scenario_name, match_position, trajectory
                        )
                        ...
                        trigger_id = RouteParser.check_trigger_position(
                            waypoint, existent_triggers
                        )
