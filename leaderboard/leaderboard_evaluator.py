...
class LeaderboardEvaluator(object):
    ...
    def _register_statistics(self, config, checkpoint, entry_status, crash_message=""):
        ...
        self.statistics_manager.save_record(
            current_stats_record, config.index, checkpoint
        )
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        self.statistics_manager.set_route(self.config.name, self.config.index)

        # Set up the user's agent, and the timer to avoid freezing the simulation
        try:
            self._agent_watchdog = Watchdog(self.args.timeout)
            ...
            if not self.sensors:
                ...
                AgentWrapper.validate_sensor_configuration(
                    self.sensors, track, self.args.track
                )
            ...
            self._register_statistics(
                self.config, self.args.checkpoint, self.entry_status, self.crash_message
            )
            self._cleanup()
            sys.exit(-1)
        ...
        try:
            ...
            if self.args.record:
                self.client.start_recorder(
                    "{}/{}_rep{}.log".format(
                        self.args.record, self.config.name, self.config.repetition_index
                    )
                )
            self.manager.load_scenario(
                self.scenario, self.agent_instance, self.config.repetition_index
            )
    ...
    def step(self):
        """
        Step through the scenario 1 tick
        """
    ...
    def run(self, args):
        ...
        route_indexer = RouteIndexer(
            args.routes, args.scenarios, args.repetitions, args.route_id
        )
...
def main():
    ...
    parser.add_argument(
        "--route-id", default="", type=str, help="Execute a specific route"
    )
    parser.add_argument(
        "--scenarios",
